...
Assumes /etc/nginx/sites-available and /etc/nginx/sites-enabled setup used.
    ...
    -p    Create a minimal proxy_pass server block - i.e. -p http://localhost:3000
...
cat <<EOF
    ...
    server {
        ...
        #ssl on;
        #ssl_certificate     /etc/ssl/xip.io/$ServerName.crt;
        #ssl_certificate_key /etc/ssl/xip.io/$ServerName.key;
...
# Output Nginx Proxy-Server Block Config
function create_proxy_block() {
cat <<EOF
    server {
        listen 80;
...
if [[ -z $ProxyServerBlock ]]; then
    # Create the Server Block config
    create_server_block > /etc/nginx/sites-available/${ServerBlockName}
else
    # Create the Proxy Block config
    create_proxy_block $ProxyServerBlock > /etc/nginx/sites-available/${ServerBlockName}
fi
